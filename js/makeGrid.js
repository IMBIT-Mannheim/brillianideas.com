...
for (i = 0; i < digitalLearningArray.length; i++) {
  ...
  for (j = 0; j < digitalLearningArray.length; j++) {
    movedInCombo[digitalLearningArray[i][0]][
      digitalLearningArray[0][j]
    ] = false;
  }
}
...
function rectOutlines($div) {
  ...
  if (isNaN(this.left)) this.left = parseInt($cssLeft);
  /**
   * @member {Number} rectOutlines#top
   */
  this.top = parseInt($cssTop.replace(/^\D+/g, ""));
  if (isNaN(this.top)) this.top = parseInt($cssTop);
  /**
    ...
    for (var i = 0; i < l; i++) {
      ...
      if (card_position.overlapsWith(e_position)) {
        ...
        if (movedInCombo[value[0]][id]) {
          card_position.left =
            card_position.left <= e_position.left
              ? e_position.right
              : e_position.left - card_position.width;
          card_position.top =
            card_position.top <= e_position.top
              ? e_position.bottom
              : e_position.top - card_position.height;
  ...
  switch (view) {
    case "digitalLearning":
      ...
      var cleanUp = function () {
        ...
        $(".flipcard, .flipcard .face", "#grid")
          .css("pointer-events", "auto")
          .css("cursor", "pointer");
        /*$('h1, h2, h3, h4 ,h5, p', '#grid').each(function(i,e){
					$(e).html(($(e).html().replace(/\s{2,}/g," ")));
				});*/
        $(".flipcard").each(function (i, e) {
          $(e).css({
            width: $(e).children(".front").outerWidth(true) + 1,
            height: $(e).children(".front").outerHeight(true) + 1,
            minWidth: $(e).children(".front").outerWidth(true) + 1,
            minHeight: $(e).children(".front").outerHeight(true) + 1,
          });
        });
      };
      /**
      ...
      var animateTiles = function () {
        ...
        $("#grid")
          .css("cursor", "pointer")
          .css("width", $display.width)
          .css("height", $display.height)
          .css("opacity", 1);
      ...
      var fillTile = function (i, obj) {
        ...
        $.ajax({
          url:
            "xml/index.php?base=categories&type=learning&detail=true&filter=" +
            $obj.attr("id"),
          complete: function (data) {
        ...
        $.ajax({
          ...
          complete: function (data) {
            if (data.status == 200) {
              ...
              alert(
                "Something went wrong. Please reload this page later. If this keeps occuring, please contact the mail stated in the imprint."
              );
            }
          },
        });
    ...
    case "imbit":
      ...
      $.when(
        ...
        $.when(
           ...
           */
          $("#WI")
            .css("left", Math.floor(0.2 * $display.width))
            .css("top", Math.floor(0.1 * $display.height))
            .attr("data-sid", "1"),
          $("#I")
            .css("left", Math.floor(0.25 * $display.width))
            .css("top", Math.floor(0.65 * $display.height))
            .attr("data-sid", "2"),
          $("#IT")
            .css("left", Math.floor(0.7 * $display.width))
            .css("top", Math.floor(0.2 * $display.height))
            .attr("data-sid", "3"),
          $("#W")
            .css("left", Math.floor(0.68 * $display.width))
            .css("top", Math.floor(0.7 * $display.height))
            .attr("data-sid", "4"),
          $("#MG")
            .css("left", Math.floor(0.35 * $display.width))
            .css("top", Math.floor(0.35 * $display.height))
            .attr("data-sid", "5"),
          $("#title_imbit")
            .css({
              left: "initial",
          ...
          $("#grid")
            ...
            .each(function (index, element) {
              ...
              deferredArray.push(
                $.ajax({
                  url:
                    "xml/index.php?base=grid&type=class&detail=true&withLink=true&filter=" +
                    $(element).children(".front").text(),
                }).done(function (data) {
