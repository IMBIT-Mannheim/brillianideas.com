...
function initDots() {
  ...
  $(".slidecard").each(function (i, e) {
    $("#dots").append(
      "<li" + (i == 0 ? ' class="current"' : "") + '><a href="#"></a></li>'
    );
  });
}
...
function newContentChangePage(left) {
  ...
  var targetDot = left ? currentDot.prev() : currentDot.next();
...
function openQuiz(idOfButtonClicked) {
  if (
    idOfButtonClicked == "quizWertkettenmodell" ||
    idOfButtonClicked == "quizSt-Galler-Management-Modell" ||
    idOfButtonClicked == "quizBusinessCanvas" ||
    idOfButtonClicked == "quizOekonomischesPrinzip" ||
    idOfButtonClicked == "quizStakeholderShareholder" ||
    idOfButtonClicked == "quizBWL-Quiz"
  ) {
